...
/* file_put_contents('paymob_dump',print_r($body, true));

exit; */


if(!(isset($gateway_data['obj']['success']) && $gateway_data['obj']['success'] == true)){
    exit;
}


$gateway_transaction_id = $gateway_data['obj']['id'];

$transaction_ref = $gateway_data['obj']['order']['merchant_order_id'];
$transaction_data = [];
...
$response_amount = ((float) $gateway_data['obj']['amount_cents']) / 100;
...
$user_currency_amount = ((float) $gateway_data['obj']['amount_cents']) / 100;//$metadata['amount']) / 100;
