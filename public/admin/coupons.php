...
ORDER BY %1$stbl_coupon_codes.date_created DESC', DB_TBL_PREFIX);
...
if(isset($_POST['add-coupon'])){
            ...
            if($_POST['coupon-min-fare'] == null){
                $_SESSION['action_error'][]    = "Please enter a value for Minimum Fare Threshold";
            }

            if((int) $_POST['coupon-discount-type'] == 0 && $_POST['coupon-max-perct-amnt'] == null){
                $_SESSION['action_error'][]    = "Please enter a value for Percentage Discount Amount Limit";
            }
            ...
            $minfare = (float) $_POST['coupon-min-fare'];
            $maxpercentagediscount = (float) $_POST['coupon-max-perct-amnt'];
            ...
            $query = sprintf('INSERT INTO %stbl_coupon_codes(max_discount_amount,min_fare,coupon_title,vehicles,coupon_code,city,discount_type,discount_value,limit_count,user_limit_count,active_date,`expiry_date`,date_created) VALUES'.
            '("%s","%s","%s","%s","%s","%d","%d","%s","%d","%d","%s","%s","%s")', 
                DB_TBL_PREFIX, 
    ...
    $query = sprintf('UPDATE %stbl_coupon_codes SET max_discount_amount = "%s",min_fare = "%s",coupon_title = "%s",vehicles = "%s",coupon_code="%s",city=%d,discount_type=%d,discount_value="%s",limit_count=%d,user_limit_count=%d,active_date="%s",`expiry_date`="%s" WHERE id=%d', 
        DB_TBL_PREFIX,
        $maxpercentagediscount,
        $minfare, 
        mysqli_real_escape_string($GLOBALS['DB'], $_POST['coupon-title']),
