...
$GLOBALS['admin_template']['page_title'] = "<i class='fa fa-user'></i> Edit Staff"; //Set the title of the page on the admin interface
...
WHERE %1$stbl_routes.r_scope = 0', DB_TBL_PREFIX); //Get required user information from DB
 ...
 if(isset($_GET['action'])){
    if($_GET['action'] == "act"){
            ...
            if(!$result = mysqli_query($GLOBALS['DB'], $query)){
                ...
                $_SESSION['action_error'][] = "An error has occured. Could not activate staff account";
                header("location: ".SITE_URL."admin/edit-staff.php"); //Yes? then redirect
                exit;
            
            }

            //send push notification to notify user about account activation
            $query  = sprintf('SELECT disp_lang,push_notification_token FROM %stbl_users WHERE user_id = %d', DB_TBL_PREFIX, $id);
            if($result = mysqli_query($GLOBALS['DB'], $query)){
                if(mysqli_num_rows($result)){
                    $row = mysqli_fetch_assoc($result);
                    $title = "Account activated";
                    $body = __("Your account has been activated. Thank you for being part of our service",[],"r|{$row['disp_lang']}");
                    $ext_msg_data = array(
                        'mode' => 3,
                        'message' => $body,
                        'status' => 1
                    ...
                    if(!empty($row['push_notification_token'])){                    
                        sendPushNotification($title,$body,$row['push_notification_token'],null,json_encode($ext_msg_data),0,null);
                    }
            ...
            $_SESSION['reload_id'] = $id;
            $_SESSION['action_success'][] = "The staff account was activated successfully.";
            header("location: ".SITE_URL."admin/all-staffs.php"); //Yes? then redirect
            exit;
        ...
        if(!$result = mysqli_query($GLOBALS['DB'], $query)){
            ...
            //echo mysqli_error($GLOBALS['DB']);
            $_SESSION['reload_id'] = $id;
            $_SESSION['action_error'][] = "An error has occured. Could not deactivate staff account";
            header("location: ".SITE_URL."admin/edit-staff.php"); //Yes? then redirect
            exit;
        ...
        if($result = mysqli_query($GLOBALS['DB'], $query)){
            if(mysqli_num_rows($result)){
                ...
                $body = __("Your account is currently deactivated. Please contact support",[],"r|{$row['disp_lang']}");
                $ext_msg_data = array(
                    'mode' => 3,
                    'message' => $body,
                    'status' => 0
        ...
        $_SESSION['reload_id'] = $id;
        $_SESSION['action_success'][] = "The staff account was deactivated successfully.";
        header("location: ".SITE_URL."admin/all-staffs.php"); //Yes? then redirect
        exit;
        ...
        if(!$result = mysqli_query($GLOBALS['DB'], $query)){ 
            $_SESSION['action_error'][] = "An error occured while trying to delete staff record from the database.";
            header("location: ".SITE_URL."admin/all-staffs.php"); //Yes? then redirect
            exit;
        ...
        $_SESSION['action_success'][] = "The staff record was successfully deleted.";
        header("location: ".SITE_URL."admin/all-staffs.php"); //Yes? then redirect
        exit;
...
$query = sprintf('SELECT *,%1$stbl_users.user_id AS staff_id FROM %1$stbl_users
...
if(empty($_POST)){ //let's render the add-new-franchise page UI'
    ...
    ob_start();
    include('../../drop-files/templates/admin/editstafftpl.php');
...
$user_photo = $staff_data['photo_file'];
}
...
$query = sprintf('SELECT user_id,email, phone FROM %stbl_users WHERE user_id != "%d" AND (email = "%s" OR phone="%s")', DB_TBL_PREFIX,$id, mysqli_real_escape_string($GLOBALS['DB'], $_POST['email']),mysqli_real_escape_string($GLOBALS['DB'], $_POST['phone']));
...
$route_id = !empty($_POST['route-city']) ? (int) $_POST['route-city'] : 1;

//OK, all good. lets store the registrant form data in the database
$verify_reg  = 0;
$query = sprintf('UPDATE %stbl_users SET route_id = "%d", firstname = "%s", lastname = "%s", email = "%s", phone = "%s", pwd_raw = "%s", password_hash = "%s", referal_code = "%s",account_type = "%d",photo_file = "%s",country_code = "%s",country_dial_code = "%s",country = "%s" WHERE user_id = "%d"', 
    DB_TBL_PREFIX, 
    $route_id,
...
if(! $result = mysqli_query($GLOBALS['DB'], $query)){
    ...
    $_SESSION['action_success'][] = "The staff record was modified successfully.";
    header("location: ".SITE_URL."admin/all-staffs.php"); //Yes? then redirect
    exit;
