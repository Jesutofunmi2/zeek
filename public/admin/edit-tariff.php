...
$query = sprintf('SELECT id,r_title,city_bound_coords FROM %stbl_routes WHERE r_scope = 0', DB_TBL_PREFIX);
...
if(isset($_GET['action']) && $_GET['action']== "delete"){
   ...
   //then delete record
       ...
       //remove route zones
       $query = sprintf('DELETE FROM %stbl_zones WHERE city_id = "%d"', DB_TBL_PREFIX, $id);
       $result = mysqli_query($GLOBALS['DB'], $query);



       //remove route documents
       $query = sprintf('DELETE FROM %stbl_documents WHERE doc_city = "%d"', DB_TBL_PREFIX, $id);
       $result = mysqli_query($GLOBALS['DB'], $query);
...
//check city boundary coords data
if(empty($_POST['city-boundary-data'])){

    if($_POST['zonetypeoption'] != "state"){
        $_SESSION['action_error'][] = "City boundary data not found. Draw a boundary around the city center or marker";
    }     

}else{
    $city_boundary_data = json_decode($_POST['city-boundary-data'], true);

    if(json_last_error() || !isset($city_boundary_data['coords'])){
        $_SESSION['action_error'][]    = "City boundary data not found. Draw a boundary around the city center or marker";
    }
}


if($_POST['zonetypeoption'] == "state" && empty($_POST['preset-pickup-city'])){
    $_SESSION['action_error'][]    = "Please select a pickup city for the inter state route";
} 
...
$pickup_city_id = !empty($_POST['preset-pickup-city']) ? (int) $_POST['preset-pickup-city'] : 0;

$city_boundary_data = json_decode($_POST['city-boundary-data'], true);
$city_radius = isset($city_boundary_data['radius']) ? (float) $city_boundary_data['radius'] : 0;

if($route_title == "World"){
    $city_radius = 6000000;
}

if($dist_unit == 1){
    //miles
    $city_radius = $city_radius * 0.62137;
}

$city_boundary_data_sanitized = htmlspecialchars(mysqli_real_escape_string($GLOBALS['DB'], $_POST['city-boundary-data']));
...
$query = sprintf('UPDATE %stbl_routes SET pickup_city_id = "%s",city_bound_coords = "%s",r_title = "%s",c_name = "%s",pick_name = "%s",drop_name = "%s",r_scope = "%s",lng = "%s",lat = "%s",pick_lng = "%s",pick_lat = "%s",drop_lng = "%s",drop_lat = "%s",dist_unit = "%d",city_radius = "%s",city_currency_id = "%d" WHERE id="%d"', 
DB_TBL_PREFIX,
$pickup_city_id,
$city_boundary_data_sanitized,
$route_title,
$city_name,
$pick_name,
$drop_name,
...
exit; */
    ...
    if(! $result = mysqli_query($GLOBALS['DB'], $query)){
        ...
        foreach($_POST['car_type'] as $cartype_id){
            ...
            $ind = (float) $_POST['ind-' . $cartype_id];
            ...
            $nind = (float) $_POST['nind-' . $cartype_id];
            ...
            $DB_values_string .= "({$cartype_id},%2\$d,{$cc},{$cpkr},{$cpmr},{$pcr},{$dcr},{$ncc},{$ncpkr},{$ncpmr},{$npcr},{$ndcr},{$pp_enabled},{$pp_start_time},{$pp_end_time},{$pp_active_days},{$pp_charge_type},{$pp_charge},{$fare_compute},{$ride_share},{$ind},{$nind}),";
