...
$GLOBALS['admin_template']['page_title'] = "<i class='fa fa-line-chart'></i> Customers Report"; //Set the title of the page on the admin interface
$GLOBALS['admin_template']['active_menu'] = "reports"; //Set the appropriate menu item active
$GLOBALS['admin_template']['active_sub_menu'] = "reports_cust"; //Set the appropriate menu item active
...
if(empty($_POST)){ //let's render the default users data analytics page UI'
    ...
    ob_start(); 
    include('../../drop-files/templates/admin/customersreporttpl.php');
...
function getRegTrendMonth($month,$year,$city){ //Function returns number of registrations for a month on a selected year
    ...
    $query = sprintf('SELECT COUNT(*), DATE(%1$stbl_users.account_create_date) AS date_string FROM %1$stbl_users
    WHERE %1$stbl_users.route_id = %4$d AND (%1$stbl_users.account_create_date BETWEEN "%3$s" AND "%2$s") AND %1$stbl_users.account_type = 1 GROUP BY DAY(%1$stbl_users.account_create_date)', DB_TBL_PREFIX,$end_date,$start_date,$city);
...
function getNumVerUsers(){ //Function returns number of registered users on platform
    $number_of_users = 0;
    //Get data from database
    $query = sprintf('SELECT COUNT(*) FROM %1$stbl_users WHERE %1$stbl_users.account_type = 1  ', DB_TBL_PREFIX);
