...
$GLOBALS['admin_template']['page_title'] = "<i class='fa fa-file-text'></i> Customer Documents"; //Set the title of the page on the admin interface
$GLOBALS['admin_template']['active_menu'] = "documents"; //Set the appropriate menu item active
$GLOBALS['admin_template']['active_sub_menu'] = "c_docs"; //Set the appropriate menu item active
...
WHERE %1$stbl_documents.doc_user = %2$d ORDER BY %1$stbl_documents.date_created DESC', DB_TBL_PREFIX, 0);
...
if(isset($_GET['status'])){ //document activate / deactivate button clicked
    ...
    if(DEMO){
        ...
        header("location: ".SITE_URL."admin/cdocuments.php"); //Yes? then redirect 
        exit;
    }
    ...
    $query = sprintf('UPDATE %stbl_documents SET `status` = %d WHERE id = %d', 
        ...
        if(! $result = mysqli_query($GLOBALS['DB'], $query)){
            ...
            ob_start();
            include('../../drop-files/templates/admin/cdocumentstpl.php'); 
                $msgs = '';
...
if(isset($_POST['add-document'])){
            ...
            $query = sprintf('SELECT * FROM %stbl_documents WHERE title = "%s" AND doc_city = %d AND doc_type = %d AND doc_user = %d', DB_TBL_PREFIX, mysqli_real_escape_string($GLOBALS['DB'], $_POST['doc-title']), $doc_city, $doc_type,0); //Get required user information from DB
