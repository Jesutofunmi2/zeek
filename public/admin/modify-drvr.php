...
if(isset($_GET['action'])){
    ...
    if($_GET['action'] == "encar"){
                
        $query = sprintf('UPDATE %stbl_drivers SET allow_vehicle_edit = 1 WHERE driver_id = "%d"', DB_TBL_PREFIX, $id );

        if(!$result = mysqli_query($GLOBALS['DB'], $query)){
            
            //echo mysqli_error($GLOBALS['DB']);
            $_SESSION['reload_id'] = $id;
            $_SESSION['action_error'][] = "An error has occured. Could not enable vehicle category change on app";
            header("location: ".SITE_URL."admin/modify-drvr.php?id={$id}"); //Yes? then redirect
            exit;
        
        }

        //send push notification to notify user about account activation
        $query  = sprintf('SELECT disp_lang,push_notification_token FROM %stbl_drivers WHERE driver_id = %d', DB_TBL_PREFIX, $id);
        if($result = mysqli_query($GLOBALS['DB'], $query)){
            if(mysqli_num_rows($result)){
                $row = mysqli_fetch_assoc($result);
                $title = WEBSITE_NAME;
                $body = __("Permission granted to change vehicle category",[],"d|{$row['disp_lang']}");
        ...
        $_SESSION['reload_id'] = $id;
        $_SESSION['action_success'][] = "Vehicle category change on app is enabled.";
        header("location: ".SITE_URL."admin/modify-drvr.php?id={$id}"); //Yes? then redirect
        exit;


    }elseif($_GET['action'] == "discar"){
            
        $query = sprintf('UPDATE %stbl_drivers SET allow_vehicle_edit = 0 WHERE driver_id = "%d"', DB_TBL_PREFIX, $id );
        ...
        $_SESSION['reload_id'] = $id;
        $_SESSION['action_success'][] = "Vehicle category change on app is disabled";
        header("location: ".SITE_URL."admin/modify-drvr.php?id={$id}"); //Yes? then redirect
        exit;

    }elseif($_GET['action'] == "enphoto"){
                
            $query = sprintf('UPDATE %stbl_drivers SET allow_photo_edit = 1 WHERE driver_id = "%d"', DB_TBL_PREFIX, $id );

            if(!$result = mysqli_query($GLOBALS['DB'], $query)){
                
                //echo mysqli_error($GLOBALS['DB']);
                $_SESSION['reload_id'] = $id;
                $_SESSION['action_error'][] = "An error has occured. Could not enable photo change on app";
            ...
            if($result = mysqli_query($GLOBALS['DB'], $query)){
                if(mysqli_num_rows($result)){
                    $row = mysqli_fetch_assoc($result);
                    $title = WEBSITE_NAME;
                    $body = __("Permission granted to change profile photo",[],"d|{$row['disp_lang']}");
            ...
            $_SESSION['reload_id'] = $id;
            $_SESSION['action_success'][] = "Profile photo change on app is enabled.";
            header("location: ".SITE_URL."admin/modify-drvr.php?id={$id}"); //Yes? then redirect
            exit;


    }elseif($_GET['action'] == "disphoto"){
            
        $query = sprintf('UPDATE %stbl_drivers SET allow_photo_edit = 0 WHERE driver_id = "%d"', DB_TBL_PREFIX, $id );

        if(!$result = mysqli_query($GLOBALS['DB'], $query)){
            
            //echo mysqli_error($GLOBALS['DB']);
            $_SESSION['reload_id'] = $id;
            $_SESSION['action_error'][] = "An error has occured. Could not disable photo change on app";
            header("location: ".SITE_URL."admin/modify-drvr.php?id={$id}"); //Yes? then redirect
            exit;
        ...
        $_SESSION['reload_id'] = $id;
        $_SESSION['action_success'][] = "Profile photo change on app is disabled";
        header("location: ".SITE_URL."admin/modify-drvr.php?id={$id}"); //Yes? then redirect
        exit;

    }elseif($_GET['action'] == "act"){
            
            $query = sprintf('UPDATE %stbl_drivers SET is_activated = 1, account_active = 1 WHERE driver_id = "%d"', DB_TBL_PREFIX, $id );
            ...
            if($result = mysqli_query($GLOBALS['DB'], $query)){
                if(mysqli_num_rows($result)){
                    $row = mysqli_fetch_assoc($result);
                    $title = WEBSITE_NAME;
                    $body = __("Your account has been activated. Thank you for being part of our service",[],"d|{$row['disp_lang']}");
        ...
        //send push notification to notify user about account deativation
        $query  = sprintf('SELECT disp_lang,push_notification_token FROM %stbl_drivers WHERE driver_id = %d', DB_TBL_PREFIX, $id);
        if($result = mysqli_query($GLOBALS['DB'], $query)){
            if(mysqli_num_rows($result)){
                $row = mysqli_fetch_assoc($result);
                $title = WEBSITE_NAME;
                $body = __("Your account is currently deactivated. Please contact support",[],"d|{$row['disp_lang']}");
        ...
        $driver_data = [];
        $query = sprintf('SELECT * FROM %stbl_drivers WHERE driver_id = "%d"',DB_TBL_PREFIX, $id );
            if($result = mysqli_query($GLOBALS['DB'], $query)){
...
$validation_res = validatePhoneNumber('+' . $_POST['country-dial-code'] . $_POST['phone']);
            
if(isset($validation_res['error'])){
    $_SESSION['action_error'][] = "Phone number is invalid";   
}


//check if email or phone number already exists
$query = sprintf('SELECT driver_id,email,phone FROM %stbl_drivers WHERE driver_id != %d AND ((country_dial_code = "%s" AND (phone= "%s" OR phone="%s")) OR email = "%s")', 
DB_TBL_PREFIX, 
$id,
'+' . $_POST['country-dial-code'],
...
$query = sprintf('UPDATE %stbl_drivers SET car_year = "%s", route_id = "%d",pwd_raw = "%s",password_hash = "%s",drv_address = "%s",email = "%s",firstname = "%s",lastname = "%s",phone = "%s",`state` = "%s",drv_country = "%s",car_plate_num = "%s",car_model = "%s",car_color = "%s",ride_id = "%s",referal_code = "%s",franchise_id = "%s",photo_file = "%s",bank_name = "%s",bank_acc_holder_name = "%s",bank_acc_num = "%s",bank_code = "%s",driver_commision = "%s",bank_swift_code = "%s",country_code = "%s",country_dial_code = "%s" WHERE driver_id = "%d"', 
    DB_TBL_PREFIX,
    $_POST['caryear'],
    (int) $_POST['carcity'], 
