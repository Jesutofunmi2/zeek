...
$GLOBALS['admin_template']['page_title'] = "<i class='fa fa-user'></i> Add Staff"; //Set the title of the page on the admin interface
$GLOBALS['admin_template']['active_menu'] = "accounts"; //Set the appropriate menu item active
$GLOBALS['admin_template']['active_sub_menu'] = "new_acc"; //Set the appropriate menu item active
...
if(empty($_POST)){ //let's render the add-new-franchise page UI'
    ...
    ob_start();
    include('../../drop-files/templates/admin/addstafftpl.php');
...
if(empty($_POST['image-data'])){
    $_SESSION['action_error'][] = "Please upload a passport photo for this staff";
}       
else{
...
$staff_country = codeToCountryName(strtoupper($_POST['country-code']));
...
$verify_reg  = 0;
$query = sprintf('INSERT INTO %stbl_users (route_id,firstname, lastname, email, phone, pwd_raw, password_hash, account_create_date,referal_code,is_activated,account_type,photo_file,country_code,country_dial_code,country) VALUES'.
'("%d","%s","%s","%s","%s","%s","%s","%s","%s","%d","%d","%s","%s","%s","%s")', 
    DB_TBL_PREFIX, 
    $route_id,
...
if(! $result = mysqli_query($GLOBALS['DB'], $query)){
    ...
    $_SESSION['action_success'][] = "The staff record was created successfully.";
    header("location: ".SITE_URL."admin/all-staffs.php"); //Yes? then redirect
    exit;
