...
function getPaymentTrendMonthCity($month,$year){ //Function returns number of registrations for a month on a selected year and city
    ...
    $start_date = $year."-".$month_formated."-01 00:00:00";
    $end_date = $year."-".$month_formated."-".$num_of_month_days . " 23:59:00";
    ...
    if($result = mysqli_query($GLOBALS['DB'], $query)){
        if(mysqli_num_rows($result)){
            while($row = mysqli_fetch_assoc($result)){
                ...
                $payment_data_bookings_completed_sort[$row['date_string']] += $row['price'];
    ...
    if($result = mysqli_query($GLOBALS['DB'], $query)){
        if(mysqli_num_rows($result)){
            while($row = mysqli_fetch_assoc($result)){
                if(empty($row['date_string']))continue;
                $payment_data_online_sort[$row['date_string']] += $row['price'];
    ...
    if($result = mysqli_query($GLOBALS['DB'], $query)){
        ...
        if(mysqli_num_rows($result)){
            while($row = mysqli_fetch_assoc($result)){
                if(empty($row['date_string']))continue;
                $payment_data_wallet_fund_sort[$row['date_string']] += $row['price'];
    ...
    if($result = mysqli_query($GLOBALS['DB'], $query)){
        if(mysqli_num_rows($result)){
            while($row = mysqli_fetch_assoc($result)){
                if(empty($row['date_string']))continue;
                $payment_data_payouts_sort[$row['date_string']] += $row['price'];
...
function getPaymentTrendMonthDrvr($month,$year){ //Function returns number of registrations for a month on a selected year
    ...
    if($result = mysqli_query($GLOBALS['DB'], $query)){
        if(mysqli_num_rows($result)){
            while($row = mysqli_fetch_assoc($result)){
                if(empty($row['date_string']))continue;
                $payment_data_bookings_completed_earnings_sort[$row['date_string']] += $row['price'];
