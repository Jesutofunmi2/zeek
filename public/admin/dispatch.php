...
WHERE %1$stbl_bookings.status = 0 AND %1$stbl_bookings.dispatch_mode = 1', DB_TBL_PREFIX);  //Get and count all data
...
INNER JOIN %1$stbl_routes ON %1$stbl_routes.id = %1$stbl_bookings.route_id
WHERE %1$stbl_bookings.dispatch_mode = 1 ORDER BY %1$stbl_bookings.date_created DESC LIMIT %2$d,%3$d', DB_TBL_PREFIX, $offset, ITEMS_PER_PAGE);
...
if($result = mysqli_query($GLOBALS['DB'], $query)){
    ...
    if(mysqli_num_rows($result)){
        while($row = mysqli_fetch_assoc($result)){
            if(!empty($row['booking_driver_alloc'])){
                $bookings_allocated[$row['booking_id']] = 1;
            }
            $dispatch_page_items[$row['booking_id']] = $row;
        }
