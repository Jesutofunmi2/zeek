...
if(!empty($_GET['filter-records'])){
  ...
  if(isset($_GET['city'])){
    ...
    if(!empty($city_val)){
      $query_modifier .= "AND " . DB_TBL_PREFIX . "tbl_drivers.route_id = {$city_val} ";
    }
  ...
  if(isset($_GET['car-category'])){

    $ride_id = (int) $_GET['car-category'];

    if(!empty($ride_id)){
      $query_modifier .= "AND " . DB_TBL_PREFIX . "tbl_drivers.ride_id = {$ride_id} ";
    }
  ...
  if(isset($_GET['act-status']) && $_GET['act-status'] != "---"){

    $act_status = (int) $_GET['act-status'];

    $query_modifier .= "AND " . DB_TBL_PREFIX . "tbl_drivers.is_activated = {$act_status} ";
  ...
  if(isset($_GET['online-status']) && $_GET['online-status'] != "---"){

    $online_status = (int) $_GET['online-status'];

    $location_info_age = gmdate('Y-m-d H:i:s', time() - LOCATION_INFO_VALID_AGE);

    if($online_status == 1){
      //get online drivers      
      $query_modifier .= "AND " . DB_TBL_PREFIX . "tbl_drivers.available = 1 AND " . DB_TBL_PREFIX . "tbl_driver_location.location_date > \"{$location_info_age}\" ";
    }elseif($online_status == 0){
      //offline drivers
      $query_modifier .= "AND " . DB_TBL_PREFIX . "tbl_driver_location.location_date < \"{$location_info_age}\" ";
    }    
  ...
  if(isset($_GET['rating'])){
    ...
    if(!empty($rating_val)){
      $query_modifier .= "AND " . DB_TBL_PREFIX . "tbl_drivers.driver_rating = {$rating_val} ";
    }
...
$_SESSION['drivers-export_query']['q_mod'] = $query_modifier;
$_SESSION['drivers-export_query']['sort'] = $sort_by; 
...
$query = sprintf('SELECT * FROM %1$stbl_rides WHERE 1 = 1 ORDER BY ride_type ASC', DB_TBL_PREFIX);
