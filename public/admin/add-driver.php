...
$query = sprintf('SELECT driver_id,email,phone FROM %stbl_drivers WHERE (country_dial_code = "%s" AND (phone= "%s" OR phone="%s")) OR email = "%s"', 
DB_TBL_PREFIX, 
...
$query = sprintf('INSERT INTO %stbl_drivers(car_year,route_id,pwd_raw,password_hash,drv_address,email,firstname,lastname,phone,`state`,drv_country,car_plate_num,car_model,car_color,ride_id,referal_code,franchise_id,account_create_date,photo_file,bank_name,bank_acc_holder_name,bank_acc_num,bank_code,driver_commision,bank_swift_code,country_code,country_dial_code) VALUES'.
...
/* //add an entry for this driver in the locations table
