...
function updateDashboard(){
    ...
    $drivers_earnings = 0.00;

    $daily_active_customers = 0;
    ...
    /* $query = sprintf('SELECT COUNT(*) FROM %1$stbl_bookings WHERE %1$stbl_bookings.date_created >= "%2$s" AND %1$stbl_bookings.date_created <= "%3$s"', DB_TBL_PREFIX,$start_date,$end_date);
    ...
    //Get the number of daily active cutomers
    
    $query = sprintf('SELECT COUNT(*) FROM %1$stbl_users WHERE %1$stbl_users.last_login_date >= "%2$s" AND %1$stbl_users.last_login_date <= "%3$s"', DB_TBL_PREFIX,$start_date,$end_date);
    ...
    if($result = mysqli_query($GLOBALS['DB'], $query)){
        ...
        $row = mysqli_fetch_assoc($result);            
        $daily_active_customers = $row['COUNT(*)'];
    ...
    $query = sprintf('SELECT COUNT(*) FROM %1$stbl_users WHERE %1$stbl_users.account_create_date >= "%2$s" AND %1$stbl_users.account_create_date <= "%3$s"', DB_TBL_PREFIX,$start_date,$end_date);
    ...
    //Get the number of driver registrations for today
    $num_driver_registrations = 0;
    $query = sprintf('SELECT COUNT(*) FROM %1$stbl_drivers WHERE %1$stbl_drivers.account_create_date >= "%2$s" AND %1$stbl_drivers.account_create_date <= "%3$s"', DB_TBL_PREFIX,$start_date,$end_date);
    ...
    if($result = mysqli_query($GLOBALS['DB'], $query)){
        ...
        $row = mysqli_fetch_assoc($result);            
        $num_driver_registrations = $row['COUNT(*)'];
    ...
    //Get the number of driver not activated
    $num_driver_not_activated = 0;
    $query = sprintf('SELECT COUNT(*) FROM %1$stbl_drivers WHERE %1$stbl_drivers.is_activated = 0', DB_TBL_PREFIX);
    ...
    if($result = mysqli_query($GLOBALS['DB'], $query)){
        ...
        $row = mysqli_fetch_assoc($result);            
        $num_driver_not_activated = $row['COUNT(*)'];
    ...
    $query = sprintf('SELECT SUM(%1$stbl_bookings.paid_amount / %1$stbl_bookings.cur_exchng_rate) AS today_earnings FROM %1$stbl_bookings WHERE %1$stbl_bookings.status = 3 AND %1$stbl_bookings.date_completed >= "%2$s" AND %1$stbl_bookings.date_completed <= "%3$s"', DB_TBL_PREFIX, $start_date,$end_date);
    ...
    if($result = mysqli_query($GLOBALS['DB'], $query)){
        if(mysqli_num_rows($result)){
            ...
            $todays_earnings = floatVal($row['today_earnings']);
    ...
    foreach($dates_data as $date_str){
        $week_earnings[$date_str] = 0;
    }


    //Get earnings for 7 day period. convert amount to base currency and sum all
    $query = sprintf('SELECT (%1$stbl_bookings.paid_amount / %1$stbl_bookings.cur_exchng_rate) AS today_earnings, DATE(%1$stbl_bookings.date_completed) AS date_string FROM %1$stbl_bookings WHERE %1$stbl_bookings.status = 3 AND %1$stbl_bookings.date_completed >= "%2$s" AND %1$stbl_bookings.date_completed <= "%3$s"', DB_TBL_PREFIX, $w_start_date,$w_end_date);
    ...
    if($result = mysqli_query($GLOBALS['DB'], $query)){
        if(mysqli_num_rows($result)){
            ...
            while($row = mysqli_fetch_assoc($result)){
                $day_earnings = floatVal($row['today_earnings']);
                $week_earnings[$row['date_string']] += $day_earnings;
            }
    ...
    //Get earnings for drivers for today. convert amount to base currency and sum all
    $query = sprintf('SELECT (%1$stbl_bookings.actual_cost / %1$stbl_bookings.cur_exchng_rate * (%1$stbl_bookings.driver_commision / 100)) AS price FROM %1$stbl_bookings 
    WHERE %1$stbl_bookings.driver_id != 0 AND %1$stbl_bookings.status = 3 AND %1$stbl_bookings.date_completed BETWEEN "%2$s" AND "%3$s"', DB_TBL_PREFIX, $start_date, $end_date);
    ...
    if($result = mysqli_query($GLOBALS['DB'], $query)){
        if(mysqli_num_rows($result)){
            ...
            while($row = mysqli_fetch_assoc($result)){            
                $drivers_earnings += floatVal($row['price']);
            }
    ...
    //Get earnings for company for today. convert amount to base currency and sum all
    $company_earnings = 0.00;
    $query = sprintf('SELECT (%1$stbl_bookings.paid_amount - %1$stbl_bookings.actual_cost) / %1$stbl_bookings.cur_exchng_rate AS deficit,%1$stbl_bookings.franchise_commision,(%1$stbl_bookings.actual_cost - (%1$stbl_bookings.actual_cost * (%1$stbl_bookings.driver_commision / 100))) / %1$stbl_bookings.cur_exchng_rate AS price,franchise_id, DATE(%1$stbl_bookings.date_completed) AS date_string FROM %1$stbl_bookings 
    ...
    //Get deficits as a result of discounts applied. convert amount to base currency and sum all
    $ride_discounts = 0.00;
    $query = sprintf('SELECT (%1$stbl_bookings.paid_amount - %1$stbl_bookings.actual_cost) / %1$stbl_bookings.cur_exchng_rate AS deficit, DATE(%1$stbl_bookings.date_completed) AS date_string FROM %1$stbl_bookings 
    WHERE %1$stbl_bookings.status = 3 AND %1$stbl_bookings.date_completed BETWEEN "%2$s" AND "%3$s"', DB_TBL_PREFIX, $start_date, $end_date);
    ...
    if($result = mysqli_query($GLOBALS['DB'], $query)){
        if(mysqli_num_rows($result)){
            while($row = mysqli_fetch_assoc($result)){
                ...
                $ride_discounts += (floatVal($row['deficit']) < 0) ? abs(floatVal($row['deficit'])) : 0;
    ...
    $num_todays_bookings = 0;
    $num_todays_bookings_completed = 0;
    $num_todays_bookings_cancelled_riders = 0;
    $num_todays_bookings_cancelled_drivers = 0;
    $num_todays_bookings_cancelled_no_driver = 0;

    $query = sprintf('SELECT `status` FROM %stbl_bookings WHERE date_created BETWEEN "%s" AND "%s"', DB_TBL_PREFIX,$start_date,$end_date);
    if($result = mysqli_query($GLOBALS['DB'], $query)){
        if(mysqli_num_rows($result)){
            while($row = mysqli_fetch_assoc($result)){
                $num_todays_bookings ++;
                if($row['status'] == 3){
                    //completed
                    $num_todays_bookings_completed++;
                }elseif($row['status'] == 2){
                    //rider cancelled
                    $num_todays_bookings_cancelled_riders++;
                }elseif($row['status'] == 4){
                    //driver cancelled
                    $num_todays_bookings_cancelled_drivers++;
                }elseif($row['status'] == 5){
                    //no driver found
                    $num_todays_bookings_cancelled_no_driver++;
                }
            }           
        }
        mysqli_free_result($result);
    ...
    $dashdata = array(
        ...
        'num_active_customers' => $daily_active_customers,
        'num_of_bookings' => $num_todays_bookings,
        'num_bookings_completed' => $num_todays_bookings_completed,
        'num_bookings_cancelled_rider' => $num_todays_bookings_cancelled_riders,
        'num_bookings_cancelled_driver' => $num_todays_bookings_cancelled_drivers,
        'num_booking_cancelled_no_driver' => $num_todays_bookings_cancelled_no_driver,
        'num_driver_regs' => $num_driver_registrations,
        'num_driver_not_activated' => $num_driver_not_activated,
        'ride_discounts' => !empty($ride_discounts) ? $default_currency_symbol.floattocurrency($ride_discounts) : $default_currency_symbol . "0.00",
        'driver_earnings' => !empty($drivers_earnings) ? $default_currency_symbol.floattocurrency($drivers_earnings) : $default_currency_symbol . "0.00",
        'company_earnings' => !empty($company_earnings) ? $default_currency_symbol.floattocurrency($company_earnings) : $default_currency_symbol . "0.00",
        'todays_earnings' => !empty($todays_earnings) ? $default_currency_symbol.floattocurrency($todays_earnings) : $default_currency_symbol . "0.00",
...
function customersautocomp(){
    ...
    if(is_numeric($term)){
        $query_modifier = "= 1 AND " .DB_TBL_PREFIX . "tbl_users.phone LIKE '%" . $term . "%' LIMIT 0,10"; 
    }else{
...
function getDriverLocation(){
    ...
    if($redis && !empty($route_id)){
        //successfully connected to redis

        $online_drivers = [];
        $redis->zRemRangeByScore("drvs_online:{$route_id}","-inf",time() - LOCATION_INFO_VALID_AGE); //remove drivers who haven't updated their location for a while

        
        if(empty($driver_id)){
            //get all drivers
            $redis_res = $redis->zRangeByScore("drvs_online:{$route_id}",time() - LOCATION_INFO_VALID_AGE,"+inf",['withscores'=>false,'limit' => [0,200]]);
            if($redis_res){
                $online_drivers = $redis->mget($redis_res);
            }
        }else{
            //get just the priority driver data
            $online_drivers[] = $redis->get("drvpos:{$driver_id}");
        }
        ...
        foreach($online_drivers as $onlinedrivers){
            ...
            $drivers_location_items[$row['driver_id']]['source'] = "Redis";
            ...
            $drivers_location_items[$row['driver_id']]['b_angle'] = $row['b_angle'];
            $drivers_location_items[$row['driver_id']]['icon'] = SITE_URL . "img/ride-icon-types/city-driver-icon-{$row['ride_id']}.png";
            ...
            $drivers_location_items[$row['driver_id']]['location_date'] = date('d/m/Y g:i A',$row['upd_time']);
    ...
    //get all vehicles
    $rides_data = [];
    $query = sprintf('SELECT * FROM %stbl_rides WHERE 1 = 1', DB_TBL_PREFIX);
    if($result = mysqli_query($GLOBALS['DB'], $query)){
        if(mysqli_num_rows($result)){
            while($row = mysqli_fetch_assoc($result)){
                $rides_data[$row['id']] = $row; 
            }
        }
    }

    if(empty($driver_id) && empty($route_id)){
        //get all drivers accross all cities
        ...
        WHERE %1$stbl_drivers.available = 1 /* AND %1$stbl_driver_location.location_date > "%2$s" */ LIMIT 200', DB_TBL_PREFIX, $location_info_age);
    }elseif($driver_id == 0 && !empty($route_id)){
        //get all drivers in the selected city
        $route_id = (int) $_POST['route_id'];
        $query = sprintf('SELECT * FROM %1$stbl_driver_location 
        INNER JOIN %1$stbl_drivers ON %1$stbl_drivers.driver_id = %1$stbl_driver_location.driver_id
        ...
        //get only the driver with the ID
        $query = sprintf('SELECT * FROM %1$stbl_driver_location 
        INNER JOIN %1$stbl_drivers ON %1$stbl_drivers.driver_id = %1$stbl_driver_location.driver_id
    ...
    if($result = mysqli_query($GLOBALS['DB'], $query)){
        ...
        if(mysqli_num_rows($result)){
            while($row = mysqli_fetch_assoc($result)){
                ...
                $drivers_location_items[$row['driver_id']]['lat'] = $row['lat'];
                $drivers_location_items[$row['driver_id']]['lng'] = $row['long'];
                $drivers_location_items[$row['driver_id']]['icon'] = isset($rides_data[$row['ride_id']]) ? SITE_URL . "img/ride-icon-types/city-driver-icon-{$rides_data[$row['ride_id']]['icon_type']}.png" : SITE_URL . "img/ride-icon-types/city-driver-icon-1.png";
                $drivers_location_items[$row['driver_id']]['name'] = $row['firstname'] . ' ' . $row['lastname'];
                $drivers_location_items[$row['driver_id']]['view_link'] = "<a href='view-driver.php?id={$row['driver_id']}' >View</a>"; 
...
function driversautocomp(){
    ...
    if($bookrecsearch){
        ...
        if(is_numeric($term)){
            $query_modifier = DB_TBL_PREFIX . "tbl_bookings.driver_phone LIKE '%" . $term . "%' LIMIT 0,10"; 
        }else{
        ...
        if(is_numeric($term)){
            $query_modifier = DB_TBL_PREFIX . "tbl_drivers.phone LIKE '%" . $term . "%' LIMIT 0,10"; 
        }else{
...
function driverscityautocomp(){

    $term = $_POST['term'];
    $bookrecsearch = !empty($_POST['bookrecsearch']) ? 1 : 0;
    $auto_complete2 = [];
    $auto_complete1 = [];
    $city = (int) $_POST['route_id'];
    ...
    if($city){
        $query = sprintf('SELECT %1$stbl_driver_location.lat,%1$stbl_driver_location.long,%1$stbl_drivers.driver_id,%1$stbl_drivers.country_dial_code,%1$stbl_drivers.phone, %1$stbl_drivers.firstname, %1$stbl_drivers.lastname FROM %1$stbl_drivers 
        INNER JOIN %1$stbl_driver_location ON %1$stbl_driver_location.driver_id = %1$stbl_drivers.driver_id
        WHERE %1$stbl_drivers.route_id = %3$d AND %1$stbl_drivers.available = 1 AND %2$s', DB_TBL_PREFIX, $query_modifier,$city);  //Get and count all data
    }else{
        $query = sprintf('SELECT %1$stbl_driver_location.lat,%1$stbl_driver_location.long,%1$stbl_drivers.driver_id,%1$stbl_drivers.country_dial_code,%1$stbl_drivers.phone, %1$stbl_drivers.firstname, %1$stbl_drivers.lastname FROM %1$stbl_drivers 
        INNER JOIN %1$stbl_driver_location ON %1$stbl_driver_location.driver_id = %1$stbl_drivers.driver_id
        WHERE %1$stbl_drivers.available = 1 AND %2$s', DB_TBL_PREFIX, $query_modifier);  //Get and count all data
    }
    ...
    if($result = mysqli_query($GLOBALS['DB'], $query)){
        if(mysqli_num_rows($result)){
            while($row = mysqli_fetch_assoc($result)){
                $auto_complete1['label'] = $row['firstname'] ." ".$row['lastname'] . " (" . $row['country_dial_code'] . " " . (!empty(DEMO) ? mask_string($row['phone']) : $row['phone']) . ")";
                $auto_complete1['value'] = array('driver_id'=>$row['driver_id'],'lat'=>$row['lat'],'long'=>$row['long']);
                $auto_complete2[] = $auto_complete1; 
...
function activateDriverAccount(){
    ...
    $activation_value = (int) $_GET['act_value'];
    $number_of_non_activated = 0;

    $query = sprintf('UPDATE %stbl_drivers SET is_activated = %d, account_active = 1 WHERE driver_id = "%d"', DB_TBL_PREFIX,$activation_value,$driver_id );

    if(!$result = mysqli_query($GLOBALS['DB'], $query)){
        $data_array = array("error"=>1);    
        echo json_encode($data_array); 
        exit;    
    }

    //send push notification to notify user about account activation / deactivation
    $query  = sprintf('SELECT disp_lang,push_notification_token FROM %stbl_drivers WHERE driver_id = %d', DB_TBL_PREFIX, $driver_id);
    if($result = mysqli_query($GLOBALS['DB'], $query)){
        if(mysqli_num_rows($result)){
            $row = mysqli_fetch_assoc($result);
            $title = WEBSITE_NAME;
            $body = $activation_value == 1 ? __("Your account has been activated. Thank you for being part of our service",[],"d|{$row['disp_lang']}") : __("Your account is currently deactivated. Please contact support",[],"d|{$row['disp_lang']}");

            if(!empty($row['push_notification_token'])){                    
                sendPushNotification($title,$body,$row['push_notification_token'],null,0,0,null);
            }
            ...
            if(!empty($row['push_notification_token'])){                    
                sendPushNotification($title,$body,$row['push_notification_token'],$ext_msg_data,0,0,null);
            }
    ...
    //Get number of non activated drivers
    $query = sprintf('SELECT COUNT(*) FROM %1$stbl_drivers
    WHERE %1$stbl_drivers.is_activated = 0', DB_TBL_PREFIX);  //Get and count all data
    ...
    if($result = mysqli_query($GLOBALS['DB'], $query)){
        ...
        if(mysqli_num_rows($result)){
            ...
            $number_of_non_activated = $row['COUNT(*)'];
    ...
    $data_array = array("success"=>1,'num_not_activated' => $number_of_non_activated);    
    echo json_encode($data_array); 
    exit;
...
function updateDocStatus(){
    ...
    $query = sprintf('UPDATE %stbl_users_documents SET u_doc_status = %d, u_can_edit = %d WHERE id = %d AND u_id = %d AND u_type = %d', DB_TBL_PREFIX,$doc_status,$user_can_edit,$doc_id,$user_id,$user_type);
    ...
    if($user_type == 1){
        //driver
        $query  = sprintf('SELECT disp_lang,push_notification_token FROM %stbl_drivers WHERE driver_id = %d', DB_TBL_PREFIX, $user_id);
        if($result = mysqli_query($GLOBALS['DB'], $query)){
            if(mysqli_num_rows($result)){
                $row = mysqli_fetch_assoc($result);
                $title = WEBSITE_NAME;
                $body = __("The status of one of your submitted documents has changed",[],"d|{$row['disp_lang']}");
                         
                
                if(!empty($row['push_notification_token'])){                    
                    sendPushNotification($title,$body,$row['push_notification_token'],null,1,0,null);
                }
        ...
        //rider
        $query  = sprintf('SELECT disp_lang,push_notification_token FROM %stbl_users WHERE `user_id` = %d', DB_TBL_PREFIX, $user_id);
        if($result = mysqli_query($GLOBALS['DB'], $query)){
            if(mysqli_num_rows($result)){
                $row = mysqli_fetch_assoc($result);
                $title = WEBSITE_NAME;
                $body = __("The status of one of your submitted documents has changed",[],"r|{$row['disp_lang']}");
...
function exportBookings(){
    ...
    if(!empty(DEMO)){
        $error = array("error"=>"Demo mode. Cannot export data");
        echo json_encode($error); 
        exit; 
    }

    if(!isset($_SESSION['bookings-export_query'])){
    ...
    $query_modifier = $_SESSION['bookings-export_query']['q_mod'];
    $sort_by = $_SESSION['bookings-export_query']['sort'];
    $bookings_page_items = [];
    $data_items = [];
    ...
    INNER JOIN %1$stbl_users ON %1$stbl_users.user_id = %1$stbl_bookings.user_id
    WHERE %1$stbl_bookings.id %2$s ORDER BY %3$s DESC', DB_TBL_PREFIX, $query_modifier, $sort_by);
    ...
    $filename = "DT-Booking-data-" . crypto_string('nozero',4);
    
    if($type == 1){        
        $res = exportExcel($filename,$data_items);
    }elseif($type == 2){
    ...
    if(!empty($res)){
        ...
        if(isset($res['error'])){
            ...
            if($type == 3){
                ...
                $success = array("success"=>"Data was exported successfully - filename: {$filename}.xls", "download" => SITE_URL . "downloadex.php?file={$filename}.xls");
            }
...
function exportsBookings(){
    ...
    if(!isset($_SESSION['sbookings-export_query'])){
        $error = array("error"=>"The selected data is not valid");
        echo json_encode($error); 
        exit; 
    }
    ...
    $type = isset($_POST['type']) ? (int) $_POST['type'] : 0;
    $query_modifier = $_SESSION['sbookings-export_query']['q_mod'];
    $sort_by = $_SESSION['sbookings-export_query']['sort'];
    $bookings_page_items = [];
    $data_items = [];
    $head_titles = array("Booking ID","Customer","Car Type","Pick-up","Drop-off","Booking Time","Est.Fare","Amount Paid","Payment Method","Assigned Driver","Status");
    ...
    $filename = "DT-Scheduled-Booking-data-" . crypto_string('nozero',4);
...
function exportCustomerReg(){
    ...
    if(!isset($_SESSION['customers-export_query'])){
        $error = array("error"=>"The selected data is not valid");
        echo json_encode($error); 
        exit; 
    }
    ...
    $type = isset($_POST['type']) ? (int) $_POST['type'] : 0;
    $query_modifier = $_SESSION['customers-export_query']['q_mod'];
    $sort_by = $_SESSION['customers-export_query']['sort'];
    $data_page_items = [];
    $export_data_items = [];
    $data_items = [];
    ...
    WHERE %1$stbl_users.account_type != 2 AND %1$stbl_users.account_type != 3 AND %1$stbl_users.account_type != 5 AND (%1$stbl_users.user_id %2$s) ORDER BY %3$s DESC', DB_TBL_PREFIX, $query_modifier, $sort_by);
    ...
    foreach($data_page_items as $customerspageitems){   
        ...
        $data_items[] = array($name,$email,$customerspageitems['country_dial_code']." ".$phone,$customerspageitems['wallet_amount'],date('l, M j, Y H:i:s',strtotime($customerspageitems['account_create_date'].' UTC')));
        $count++;
        
    }

    
    $filename = "DT-Customer-Reg-data-" . crypto_string('nozero',4);
...
function exportDriverReg(){
    ...
    if(!isset($_SESSION['drivers-export_query'])){
        $error = array("error"=>"The selected data is not valid");
        echo json_encode($error); 
        exit; 
    }
    ...
    $query_modifier = $_SESSION['drivers-export_query']['q_mod'];
    $sort_by = $_SESSION['drivers-export_query']['sort'];
    $data_page_items = [];
    $export_data_items = [];
    $data_items = [];
    ...
    WHERE %1$stbl_drivers.driver_id %2$s ORDER BY %3$s', DB_TBL_PREFIX, $query_modifier, $sort_by);
    ...
    foreach($data_page_items as $driverspageitems){    
        ...
        $data_items[] = array($drvr_id,$driver_name,$driverspageitems['country_dial_code']." ".$phone,$driverspageitems['r_title'],$driverspageitems['wallet_amount'],$driverspageitems['car_model'],$driverspageitems['car_plate_num']);
        $count++;
        
    }

    
    $filename = "DT-Driver-Reg-data-" . crypto_string('nozero',4);
...
function exportWalletFundData(){
    ...
    if(!isset($_SESSION['walletfund-export_query'])){
        $error = array("error"=>"The selected data is not valid");
        echo json_encode($error); 
        exit; 
    }
    ...
    $type = isset($_POST['type']) ? (int) $_POST['type'] : 0;
    $query_modifier = $_SESSION['walletfund-export_query']['q_mod'];
    $sort_by = $_SESSION['walletfund-export_query']['sort'];
    $data_page_items = [];
    $export_data_items = [];
    $data_items = [];
    $head_titles = array("Type","Details","Amount funded","Wallet balance","Date","Comment");

    $query = sprintf('SELECT * FROM %1$stbl_wallet_fund WHERE id %2$s ORDER BY %3$s DESC', DB_TBL_PREFIX,$query_modifier,$sort_by);
    ...
    $filename = "DT-Wallet-Funding-data-" . crypto_string('nozero',4);
...
function exportPayoutsData(){
    ...
    if(!isset($_SESSION['payouts-export_query'])){
        $error = array("error"=>"The selected data is not valid");
        echo json_encode($error); 
        exit; 
    }
    ...
    $type = isset($_POST['type']) ? (int) $_POST['type'] : 0;
    $query_modifier = $_SESSION['payouts-export_query']['q_mod'];
    $sort_by = $_SESSION['payouts-export_query']['sort'];

    $data_page_items = [];
    $export_data_items = [];
    $data_items = [];
    $head_titles = array("User","Account type","Amount","Wallet Amount (Old)","Wallet Balance (New)","Account Name","Account Number","Bank Name","Status","Date Requested","Date Processed");

    $query = sprintf('SELECT %1$stbl_wallet_withdrawal.*,%1$stbl_wallet_withdrawal.id AS wid,%1$stbl_drivers.firstname,%1$stbl_drivers.lastname,%1$stbl_drivers.country_dial_code,%1$stbl_drivers.phone,%1$stbl_franchise.franchise_name,%1$stbl_franchise.franchise_phone,%1$stbl_franchise.bank_acc_holder_name AS faccount_name,%1$stbl_franchise.bank_acc_num AS faccount_number,%1$stbl_franchise.bank_name AS fbank_name,%1$stbl_drivers.bank_acc_holder_name AS daccount_name,%1$stbl_drivers.bank_acc_num AS daccount_number,%1$stbl_drivers.bank_name AS dbank_name FROM %1$stbl_wallet_withdrawal 
    ...
    WHERE %2$s ORDER BY %3$s DESC', DB_TBL_PREFIX, $query_modifier,$sort_by);
    ...
    if($result = mysqli_query($GLOBALS['DB'], $query)){
        ...
        if(mysqli_num_rows($result)){
            ...
            $error = array("error"=>"No data availables");
            echo json_encode($error); 
            exit; 
        }
    ...
    foreach($data_page_items as $withdrawalrequestsdata){
        ...
        if($withdrawalrequestsdata['user_type']){
            ...
            $account_name = !empty($withdrawalrequestsdata['faccount_name']) ? $withdrawalrequestsdata['faccount_name'] : "N/A";
            $account_number = !empty($withdrawalrequestsdata['faccount_number']) ? $withdrawalrequestsdata['faccount_number'] : "N/A";
            $bank_name = !empty($withdrawalrequestsdata['fbank_name']) ? $withdrawalrequestsdata['fbank_name'] : "N/A";
        }else{
            ...
            $account_name = !empty($withdrawalrequestsdata['daccount_name']) ? $withdrawalrequestsdata['daccount_name'] : "N/A";
            $account_number = !empty($withdrawalrequestsdata['daccount_number']) ? $withdrawalrequestsdata['daccount_number'] : "N/A";
            $bank_name = !empty($withdrawalrequestsdata['dbank_name']) ? $withdrawalrequestsdata['dbank_name'] : "N/A";
        }
        ...
        $data_items[] = array($user,$user_type_name,$withdrawalrequestsdata['cur_symbol']. $withdrawalrequestsdata['withdrawal_amount'],$default_currency_symbol. $withdrawalrequestsdata['wallet_amount'],$default_currency_symbol . $withdrawalrequestsdata['wallet_balance'],$account_name,$account_number,$bank_name,$payout_status,date('l, M j, Y H:i:s',strtotime($withdrawalrequestsdata['date_requested'].' UTC')), $date_settled);
        $count++;
        
    }

    
    $filename = "DT-Payouts-data-" . crypto_string('nozero',4);
...
function exportCSV($exportfilename,$data){
    ...
    fputs($f, $bom =( chr(0xEF) . chr(0xBB) . chr(0xBF) )); //fix UTF-8 characters not rendered correctly
...
function exportExcel2($exportfilename,$data){
    ...
    $f = fopen(FILES_FOLDER . "/uploads/exports/{$exportfilename}.xls", "w");
    ...
    foreach($data as $dt){
        ...
        foreach($dt as $col => $val){
            if(empty($val))$val = "---";
            //$val = mb_convert_encoding($val,'utf-16','utf-8');
            //$val = mb_convert_encoding($val, "UTF-16", mb_detect_encoding($val, "UTF-8, ISO-8859-1, ISO-8859-15", true));
            $val = mb_convert_encoding($val,'UTF-16','UTF-8'); 
            if($row == 0){
                //add header row
                _xlsWriteCell($row, $col, $val, $f);
...
function exportExcel($exportfilename,$data){
...
function closeuserchatsupport(){

    if(empty($_SESSION['loggedin'])){
        $error = array("error"=> "Please login to continue");
        echo json_encode($error); 
        exit; 
    }
    ...
    $query = sprintf('UPDATE %stbl_chatsupport SET session_status = 0 WHERE `user_id` = %d OR rider_recipient_id = %d', DB_TBL_PREFIX, $user_id, $user_id);
...
function senduserchatsupportmsg(){
    ...
    $chat_messages_html = '';
    $chat_msg = mysqli_real_escape_string($GLOBALS['DB'], $_POST['msg']);
    $user_id = (int) $_POST['userid'];
    ...
    $query = sprintf('INSERT INTO %stbl_chatsupport (chat_msg,`rider_recipient_id`,session_status,date_created) VALUES ("%s","%d","%d","%s")',
        DB_TBL_PREFIX,
        $chat_msg,
        $user_id,
        1,
    ...
    $query = sprintf('SELECT %1$stbl_chatsupport.*, %1$stbl_users.firstname FROM %1$stbl_chatsupport 
    INNER JOIN %1$stbl_users ON %1$stbl_users.user_id = %1$stbl_chatsupport.user_id OR %1$stbl_users.user_id = %1$stbl_chatsupport.rider_recipient_id
    WHERE %1$stbl_chatsupport.user_id = %2$d OR %1$stbl_chatsupport.rider_recipient_id = %2$d ORDER BY %1$stbl_chatsupport.date_created ASC', DB_TBL_PREFIX, $user_id);
        ...
        if(mysqli_num_rows($result)){ 
            ...
            while($row = mysqli_fetch_assoc($result)){
                ...
                if($row['user_id'] != 0){
                    ...
                    $chat_messages_html .= "<div style='text-align:left;margin-top: 5px;'><div style='background-color:#1e88e5;width:55%;border-radius:10px;padding:5px;display:inline-block'><div><i style='font-size: 22px;color: white;vertical-align: middle;' class='fa fa-user-circle-o'></i> <span style='font-weight:bold; font-size:14px;color:white;'>{$row['firstname']}</span></div><p style='margin: 5px 0;text-align: left;font-size: 14px;color:white;'>{$row['chat_msg']}</p><p style='margin: 5px 0;text-align: left;font-size: 10px;color:white;'>{$date_created}</p></div></div>";

                }elseif($row['rider_recipient_id'] != 0){
                    //admin chat message
...
function getuserchatsupportmsg(){
    ...
    $query = sprintf('SELECT %1$stbl_chatsupport.*, %1$stbl_users.firstname FROM %1$stbl_chatsupport 
    INNER JOIN %1$stbl_users ON %1$stbl_users.user_id = %1$stbl_chatsupport.user_id OR %1$stbl_users.user_id = %1$stbl_chatsupport.rider_recipient_id
    WHERE %1$stbl_chatsupport.user_id = %2$d OR %1$stbl_chatsupport.rider_recipient_id = %2$d ORDER BY date_created ASC', DB_TBL_PREFIX, $user_id);
    ...
    if(isset($_SESSION['chat_support_msg_count_users']) && isset($_SESSION['chat_support_msg_count_users'][$user_id])){
        if($count > $_SESSION['chat_support_msg_count_users'][$user_id]){
            $new_message = 1;
            $_SESSION['chat_support_msg_count_users'][$user_id] = $count;
        }
    }else{
        $_SESSION['chat_support_msg_count_users'][$user_id] = 0;
        $new_message = 0;
    }
...
function closedriverchatsupport(){
    ...
    $query = sprintf('UPDATE %stbl_chatsupport SET session_status = 0 WHERE `driver_id` = %d OR driver_recipient_id = %d', DB_TBL_PREFIX, $user_id, $user_id);
...
function senddriverchatsupportmsg(){
    ...
    //save chat message in DB
    $query = sprintf('INSERT INTO %stbl_chatsupport (chat_msg,`driver_recipient_id`,session_status,date_created) VALUES ("%s","%d","%d","%s")',
        DB_TBL_PREFIX,
        $chat_msg,
        $user_id,
        1,
    ...
    //get all previous chats 
    $query = sprintf('SELECT %1$stbl_chatsupport.*, %1$stbl_drivers.firstname FROM %1$stbl_chatsupport 
    INNER JOIN %1$stbl_drivers ON %1$stbl_drivers.driver_id = %1$stbl_chatsupport.driver_id OR %1$stbl_drivers.driver_id = %1$stbl_chatsupport.driver_recipient_id
    WHERE %1$stbl_chatsupport.driver_id = %2$d OR %1$stbl_chatsupport.driver_recipient_id = %2$d ORDER BY %1$stbl_chatsupport.date_created ASC', DB_TBL_PREFIX, $user_id);
    ...
    if($result = mysqli_query($GLOBALS['DB'], $query)){
        if(mysqli_num_rows($result)){ 
            ...
            while($row = mysqli_fetch_assoc($result)){
                ...
                }elseif($row['driver_recipient_id'] != 0){
                    //admin chat message
...
function getdriverchatsupportmsg(){
    ...
    $query = sprintf('SELECT %1$stbl_chatsupport.*, %1$stbl_drivers.firstname FROM %1$stbl_chatsupport 
    INNER JOIN %1$stbl_drivers ON %1$stbl_drivers.driver_id = %1$stbl_chatsupport.driver_id OR %1$stbl_drivers.driver_id = %1$stbl_chatsupport.driver_recipient_id
    WHERE %1$stbl_chatsupport.driver_id = %2$d OR %1$stbl_chatsupport.driver_recipient_id = %2$d ORDER BY date_created ASC', DB_TBL_PREFIX, $user_id);
    ...
    if(isset($_SESSION['chat_support_msg_count_drivers']) && isset($_SESSION['chat_support_msg_count_drivers'][$user_id])){
        if($count > $_SESSION['chat_support_msg_count_drivers'][$user_id]){
            $new_message = 1;
            $_SESSION['chat_support_msg_count_drivers'][$user_id] = $count;
        }
    }else{
        $_SESSION['chat_support_msg_count_drivers'][$user_id] = 0;
        $new_message = 0;
    }
